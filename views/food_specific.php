...
if (isset($_POST['import'])) {
    ...
    if (!empty($_FILES['file']['name'] && $extension == 'csv')) {
        ...
        while (($csvData = fgetcsv($csvfile)) !== FALSE) {
            ...
            $food_specific_unit_abbreviation = trim($csvData[4]);
            $food_specific_unit_description = trim($csvData[5]);
            $grams_per_food_specific_unit = trim($csvData[6]);

            $cntSQL = "SELECT count(*) as count FROM `wp_food_specific_unit` where sku_formid='" . $sku_formid . "'";
            $record = $wpdb->get_results($cntSQL, OBJECT);
            if ($record[0]->count == 0) {

                // Insert Record

                $wpdb->insert('wp_food_specific_unit', array(
                    'food_id' => $food_id,
                    'sku_formid' => $sku_formid,
                    'key_list' => $key_list,
                    'food_description' => $food_description,
                    'food_specific_unit_abbreviation' => $food_specific_unit_abbreviation,
                    'food_specific_unit_description' => $food_specific_unit_description,
                    'grams_per_food_specific_unit' => $grams_per_food_specific_unit
                ));
            }
            $totalInserted++;
        }
    ...
    <div class="container">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">Import CSV Food Specific Unit</h3>
            </div>
